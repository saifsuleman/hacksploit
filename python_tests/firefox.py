...
class NSSInteraction(object):
    ...
    def load_profile(self, profile):
        ...
        if e != 0:
            LOG.error(
                "Couldn't initialize NSS, maybe '%s' is not a valid profile?", self.profile)
    ...
    def authenticate(self, interactive):
        ...
        try:
            ...
            if password:
                ...
                e = self.NSS._PK11_CheckUserPassword(
                    keyslot, password.encode(LIB_ENCODING))
    ...
    def decrypt_passwords(self, export, output_format="human", csv_delimiter=";", csv_quotechar="|"):
        ...
        for url, user, passw, enctype in credentials:
            ...
            LOG.debug(
                "Decoding username '%s' and password '%s' for website '%s'", user, passw, url)
            LOG.debug("Decoding username '%s' and password '%s' for website '%s'", type(
                user), type(passw), type(url))
            ...
            if output_format == "csv":
                ...
                if PY3:
                    ...
                    csv_writer.writerow({k: v.encode(USR_ENCODING)
...
def ask_password(profile, interactive):
    ...
    if sys.stdin.isatty() and interactive:
        ...
        if sys.stdin in select.select([sys.stdin], [], [], 0)[0]:
            ...
            LOG.warning(
                "Master Password not provided, continuing with blank password")
            passwd = ""
    ...
    try:
        ...
        if not interactive:
            ...
            if choice and len(choice) == 1:
                ...
                LOG.error(
                    "Don't know which profile to decrypt. We are in non-interactive mode and -c/--choice is missing.")
...
def main():
    ...
    if args.tabular:
        LOG.warning(
            "--tabular is deprecated. Use `--format csv --delimiter \\t` instead")
    ...
    if args.export_pass:
        ...
        export_pass(to_export, args.pass_cmd,
                    args.pass_prefix, username_prefix)
